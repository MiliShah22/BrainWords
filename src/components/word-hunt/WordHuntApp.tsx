...
import styles from "./WordHuntApp.module.css";
import { LEVELS, WORD_POOLS } from "./data";
import type { Pos } from "./types";
import { buildPuzzle } from "./puzzle";
import { useSelector, useDispatch } from 'react-redux';
import { RootState, AppDispatch } from '@/store';
import { setUnlocked, setStars, setProgress } from '@/store/gameSlice';
// import type { GameProgress } from '@/store/gameSlice';
...
export function WordHuntApp() {
  ...
  // Redux handles persistence via redux-persist
  ...
  function computeStars(ms: number) {
    ...
    // Simple thresholds that scale with grid size + word count.
    const base = 12_000; // 12s baseline
    const perWord = 5_000; // +5s each word
    const perCell = 120; // +0.12s per grid cell
  ...
  function onPointerUp() {
    ...
    // cfg currently unused, kept to mirror original structure
    void cfg;
  }
  ...
  return (
    <div className={`${styles.appShell} ${entranceActive ? 'animate-shell-entrance' : ''}`}>
      <div className={styles.app} aria-label="Word Hunt Adventure">
